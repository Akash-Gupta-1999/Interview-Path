...
int knapsack_rec(vector<int> profit,vector<int> weight,int n,int w){
    ...
    if(weight[n-1] > w)
        ...
        return max(profit[n-1]+knapsack_rec(profit,weight,n-1,w-weight[n-1]),knapsack_rec(profit,weight,n-1,w));
}
...
int knapsack_mem(vector<int> profit,vector<int> weight,int n,int w,vector<vector<int>> dp){
    ...
    if(weight[n-1] > w){
        ...
        dp[n-1][w] = max(profit[n-1]+knapsack_rec(profit,weight,n-1,w-weight[n-1]),knapsack_rec(profit,weight,n-1,w));
        return dp[n-1][w];
    }
}
