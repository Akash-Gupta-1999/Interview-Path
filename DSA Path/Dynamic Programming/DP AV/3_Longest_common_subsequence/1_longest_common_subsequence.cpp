...
int lcs_dp(string x,string y,int n1,int n2,vector<vector<int>>& dp){
    if(n1 == 0 || n2 == 0)
        return 0;
    if(dp[n1][n2]!=-1)
        return dp[n1][n2];
    if(x[n1-1] == y[n2-1]){
        dp[n1][n2] = 1+lcs_dp(x,y,n1-1,n2-1,dp);
        return dp[n1][n2];
    }
    dp[n1][n2] = max(lcs_dp(x,y,n1-1,n2,dp),lcs_dp(x,y,n1,n2-1,dp));
    return dp[n1][n2];
}
