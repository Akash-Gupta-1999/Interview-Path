...
private:
    ...
    // Move element at idx UP until min-heap property is satisfied
    ...
    // Move element at idx DOWN until min-heap property is satisfied
    void shiftdown(int idx) {
        int min_idx = idx; // keep track of smallest so far
        if (left_child(idx) < pq.size() && pq[left_child(idx)] < pq[min_idx])
            min_idx = left_child(idx);
        if (right_child(idx) < pq.size() && pq[right_child(idx)] < pq[min_idx])
            min_idx = right_child(idx);
        if (idx != min_idx) {
            swap(pq[idx], pq[min_idx]);
            shiftdown(min_idx); // continue down
        }
    }
...
public:
    ...
    void changepriority(int idx, int num) {
        ...
        int old_val = pq[idx];
        pq[idx] = num;
        if (num < old_val)  // if decreased, bubble up
            shiftupward(idx);
        else                // if increased, bubble down
            shiftdown(idx);
    }
...
int main() {
    ...
    p.changepriority(2, 1); // index 2 decreased to 1
    p.print();

    cout << "\n=== Get min without extract ===" << endl;
    cout << "Current min: " << p.getmin() << endl;
