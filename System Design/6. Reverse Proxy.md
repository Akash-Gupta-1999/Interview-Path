-   What Is a Reverse Proxy?
    ...
    - From the client’s perspective, the reverse proxy is the web server — clients never directly connect to backend servers.  
...
-   Why Is It Called Reverse Proxy?
    ...
    - Clients don’t know about backend servers → proxy hides them (“reverse” direction).  
...
-   Reverse Proxy vs Load Balancer
    ...
    | Works with single server? | ✔ Yes                      | ✘ No                                   |
    | Hides backend servers? | ✔ Yes                        | ✔ Usually                              |
    | Caches responses?| ✔ Yes                                | ✘ No                                   |
    | SSL termination? | ✔ Yes                                | ✔ Yes                                  |
    | Key goal         | Abstraction + control                | Avoid overloaded servers               |

    👉 In real systems, reverse proxies often include load‑balancing features.  
    Examples: Nginx, HAProxy, Envoy, Cloudflare.

-   Why Is a Reverse Proxy Important?
    - ✔ Improves site reliability: detects server failure, routes traffic to healthy servers.  
    - ✔ Enables cloud architectures:  
        - Cloud bursting (overflow traffic to cloud).  
        - Split‑application architectures (hybrid on‑prem + cloud).  
...
-   How a Reverse Proxy Works (Technical Flow)
    ...
    A. Simple Forwarding (Pass‑through)
    - Accepts request.  
...
-   How They Work Together in Modern Architecture ?
    ...
    -   CDN serves cached content directly from nearest edge node → fast response.
    -   If dynamic content or uncached file → request goes to reverse proxy:
    ...
    -   Backend server generates content → proxy returns response → client.
